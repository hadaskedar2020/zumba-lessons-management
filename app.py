...
def get_table_id_and_description(table_name, exclude_value=None):
    ...
    if exclude_value is not None:
        all_data = session.query(Table.id, Table.name).filter(Table.id != exclude_value).order_by(Table.name).all()
    else:
        all_data = session.query(Table.id, Table.name).order_by(Table.name).all()
    session.close()
    return all_data
...
def song_searcher():
    ...
    if mode == "Search" and performer_id == "None" and genre_id == "None":
       display_message = "--- Choose at least 1 search criteria"
        ...
        if mode == "Search" and len(found_data) == 0:
            display_message = "--- No matching songs found."
